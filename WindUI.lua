...
local glowStroke = am:FindFirstChildWhichIsA("UIStroke", {Thickness = 8})
local lightStroke = am:FindFirstChildWhichIsA("UIStroke", {Thickness = 2})

if glowStroke and lightStroke then
    local glowGradient = glowStroke.UIGradient
    local lightGradient = lightStroke.UIGradient
    local flowConnection
    
    flowConnection = game:GetService("RunService").Heartbeat:Connect(function(delta)
        local rotation = (glowGradient.Rotation + 60 * delta) % 360
        glowGradient.Rotation = rotation
        lightGradient.Rotation = rotation
    end)
    
    am.Destroying:Connect(function()
